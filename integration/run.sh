...
# We don't lock the OpenTofu workspace to avoid bugs such as
...
tofu init -backend-config="conn_str=$POSTGRES_CONNECTION_STRING" -backend-config="schema_name=$POSTGRES_SCHEMA_NAME" -lock=false
